...
const AskAI = () => {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      const prompt = `Lên kế hoạch chuyến đi ${days} ngày đến ${city} cùng với ngân sách ${budget}. Tập trung vào những hoạt động ${mood}`;
      const result = await model.generateContent(prompt);
      const aiResponse = await result.response.text();
      const formatedResponse = formatResponseText(aiResponse);
      setResponse(formatedResponse);
    } catch (error) {
      setResponse("Hướng dẫn bởi AI thất bại. Hãy thử lại!");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Paper className={classes.container} id="ai-section">
      <Typography variant="h5" gutterBottom>
        <b>Hãy để Travel Yoda lên kế hoạch cho chuyến đi hoàn hảo của bạn</b>
      </Typography>
            ...
            <Box>
              ...
              >
                <TextField
                  ...
                  required
                >
                  <MenuItem value="Economic">Thuộc kinh tế</MenuItem>
                  <MenuItem value="Normal">Bình thường</MenuItem>
                  <MenuItem value="Luxury">Sang trọng</MenuItem>
                </TextField>
                <Box className={classes.buttonContainer}>
                  <Button onClick={handleBack} className={classes.button}>
                    Trước đó
